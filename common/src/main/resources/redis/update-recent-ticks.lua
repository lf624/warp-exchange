...
if not lastSeqId or tonumber(lastSeqId) < tonumber(seqId) then
    -- 广播
    ...
    len = redis.call('RPUSH', LIST_RECENT_TICKS, unpack(ticks)) -- RPUSH 命令将一个或多个值追加到列表的右端
    if len > 100 then
        -- 裁剪LIST以保存最新的100个Tick:
        redis.call('LTRIM', LIST_RECENT_TICKS, len-100, len-1) -- LTRIM 裁剪列表，只保留指定范围内的元素
    end
    return true
end
-- 无更新返回false
